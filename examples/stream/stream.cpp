...
int main(int argc, char ** argv) {
    ...
    // Used to check the previous timestamps and update/replace the output
    // if the timestamps match which indicates that the same context was
    // processed a second time but perhaps with more tokens now.
    std::vector<int64_t> previous_t0;
    previous_t0.reserve(8);
            ...
            // print result;
            {
                ...
                for (int i = 0; i < n_segments; ++i) {
                    ...
                    if (params.no_timestamps) {
                        ...
                        auto it = std::find(previous_t0.begin(), previous_t0.end(), t0);
                        if (it != previous_t0.end()) {
                            // Found existing timestamp - need to overwrite from that position
                            int positions_from_end = std::distance(it, previous_t0.end());

                            if (positions_from_end > 0) {
                                // Move cursor up to the line we want to overwrite
                                printf("\33[%dA", positions_from_end);
                                // Clear from cursor to end of screen
                                printf("\33[0J");
                            }

                            // Remove all timestamps from this position onward (we'll re-add them)
                            previous_t0.erase(it, previous_t0.end());
                        }

                        previous_t0.push_back(t0);
