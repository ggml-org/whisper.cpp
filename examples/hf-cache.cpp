...
// whisper.cpp does not vendor llama.cpp's log.h; route the ported logging to stderr.
...
// local string helpers (whisper's examples/common.h lacks llama.cpp's string_* utilities)
...
// mirrors llama.cpp's common_get_model_endpoint(): MODEL_ENDPOINT / HF_ENDPOINT, default HuggingFace
static std::string get_model_endpoint() {
    const char * endpoint_env = std::getenv("MODEL_ENDPOINT");
